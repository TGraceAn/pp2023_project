...
class AddWindow(Tk):
    ...
    def add_widget(self, row_names: list[str], padx=4, pady=2):
        ...
        for index_row in range(0, len(row_names)):
            ...
            elif row_names[index_row].upper() == "VISITDATE":
                entry_vd = Entry(frame)
                entry_vd.pack()
                entry_vd.insert(0,"yyyy-mm-dd")
                entry_vd.bind('<FocusOut>', lambda x: on_focus_out(entry_vd, "yyyy-mm-dd"))
                entry_vd.bind('<FocusIn>', lambda x: on_focus_in(entry_vd))
                self.entrys.append(entry_vd)
            elif row_names[index_row].upper() == "VISITORFULLNAME":
                entry_vn = Entry(frame)
                entry_vn.pack()
                entry_vn.insert(0,"Enter person's name")
                entry_vn.bind('<FocusOut>', lambda x: on_focus_out(entry_vn, "Enter person's name"))
                entry_vn.bind('<FocusIn>', lambda x: on_focus_in(entry_vn))
                self.entrys.append(entry_vn)
            else:
                entry = Entry(frame)
                entry.pack()
