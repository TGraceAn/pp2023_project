...
class Management:
    def __init__(self, root, navs, args):
        self.window = root
        ...
        self.user_frame = Frame(self.window, bg="#228B22", height= 100)
        self.user_frame.pack(side=TOP, fill=X)

        self.employee_side_frame = Frame(self.user_frame, bg = "#228B22", width = 20)
        self.employee_side_frame.pack(side = RIGHT)

        self.employee_frame =  Label(self.user_frame , text=f"EMPLOYEE: {args}", bg = "#228B22" ,fg="white", font =('Playfair', 18,'bold'))
        self.employee_frame.pack(side=RIGHT)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--employee", type=str, required=True, help="Employee who's using this")
    args = parser.parse_args()
    temp = args.employee
    print(f"Pass Success {temp}")
    app = Management(window, navs, temp)
    window.mainloop()